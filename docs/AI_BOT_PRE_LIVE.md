...
| G5 | Drawdown halt works in live too: peak equity persists in `progress.json` and is re-read each wakeup; ≥10% DD → entries rejected, closes only — and the halt **releases** by two defined routes, both after a cool-off of `dd_cooldown_hours` has actually been served: back inside the limit → release; still below it → the peak is re-armed to the current equity (bounded by `dd_rearm_limit` per `dd_rearm_window_days`). So a halted account always resumes instead of staying dead. The re-arm does **not** wait for a flat book (this halt never force-closes; heat counts the open position, which is what bounds the next entry), and a spent re-arm budget can hold the account flat for up to roughly one `dd_rearm_window_days` before the window rolls. The backtest found the old halt refused 12,588 entries over four and a half years after the trend rule's last trade in 2022 | `_risk_snapshot` + `_drawdown_halt` + `next_progress`; 17 tests in `tests/test_ai_dd_rearm.py` |
...
| G9 | The drawdown peak is a true high-water mark in **both** modes: `_risk_snapshot` raises it (`max(peak, equity)`) every wakeup. Live has no paper ledger to raise it, and the value in `progress.json` used to be frozen at the first wakeup's equity — measured $97 → $120 → $150 with `peak_equity` pinned at 97.0, so the 10% halt measured from a stale baseline | `_risk_snapshot`; `tests/test_ai_dd_rearm.py::test_the_live_peak_tracks_new_highs_and_still_trips` |
...
|---|---|---|---|
| Y1 | The peak only sees *observed* highs | A spike inside a wakeup gap (up to 4h) is never observed, so the high-water mark is the highest equity seen at a wakeup, not the true intrawakeup high. Both modes now behave the same way (G9) | Inherent to a poll-based bot, and conservative: the halt is measured from the last observed high, never from an unobserved one |
...
| R4 | **Shared `data_dir` seeds live risk state from paper history** | `progress.json` (peak, day anchors) and the journal carry over. A live account smaller than the paper peak trips an *instant* drawdown halt on its first wakeup — no longer permanent (once the `dd_cooldown_hours` cool-off is served the peak is re-armed), but it burns one of the window's re-arms before a single live trade; paper errors pollute the live audit trail | Live runs with a **fresh, separate** `data_dir` (e.g. `data/ai_bot_live`); never `rm` paper history |
